...
clear all;
close all;
image=imread('C:\Users\Zrl\Desktop\眼睛血管相关图片和matlab代码文件\用于测试的图片\06_test.tif');    %导入图片，试了很多张效果都还可以，这已经是最好的效果了。
%% 1.选取绿色通道
imager = image(:,:,1);%红
imageg = image(:,:,2);%绿
imageb = image(:,:,3);%蓝
...
binary_data1 = imbinarize(im_adjust,threshold2);%对图像进行二值化
subplot(223);
imshow(binary_data1);
title('二值图');

%% 4.去轮廓操作:最好的操作匹配滤波后加上gabor滤波，那样才是最完美的，可惜实现不了。
img = imread('C:\Users\Zrl\Desktop\眼睛血管相关图片和matlab代码文件\用于测试的图片\05_test.tif');
img = rgb2gray(img);
[m, n] = size(img);
...
subplot(223);
binary_data2 = imbinarize(new_img,threshold);%对图像进行二值化,手动调参
imshow(binary_data2);
title('轮廓二值图');
